...
export interface KiroPathFilter {
  isIgnoredPath(filePath: string): boolean;
  filterDiff(diff: string): string;
}

export function createKiroPathFilter(repoRoot: string): KiroPathFilter {
  const rules = readKiroIgnoreRules(repoRoot);
  return {
...
function readKiroIgnoreRules(repoRoot: string): string[] {
  const ignorePath = path.join(repoRoot, ".kiroignore");
  try {
    return fs
...
function globMatch(value: string, pattern: string): boolean {
  const doubleStarPlaceholder = "__KIRO_DOUBLE_STAR__";
