...
describe("Kiro MCP tool handlers", () => {
  it("joins, records a plan, checkpoints risk, and fetches queued intervention", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-"));
    ...
    expect(checkpointWithDirection.notifications).toContain(
      "Kiro delivered 1 queued direction for this session."
    );
  ...
  it("defaults split ownership from agent chat to the recording session owner", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-default-owner-"));
    const store = createKiroStore(path.join(dir, "kiro.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
  ...
  it("keeps manual agents waiting across timeout and delivers a later decision", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-manual-handoff-"));
    const store = createKiroStore(path.join(dir, "kiro.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
  ...
  it("does not keep pausing or re-offering choices after a conflict decision is active", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-decided-"));
    const store = createKiroStore(path.join(dir, "kiro.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
  ...
  it("downgrades conflicts that include an integration session to notices", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-integration-"));
    const store = createKiroStore(path.join(dir, "kiro.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
  ...
  it("keeps adapters waiting for owner publication and delivers the published shape", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-publication-"));
    const store = createKiroStore(path.join(dir, "kiro.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
  ...
  it("waits for queued directions and times out with current choices", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "kiro-mcp-wait-"));
    const store = createKiroStore(path.join(dir, "kiro.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
