...
class BetterSqliteKiroStore implements KiroStore {
  ...
  upsertGitHubMemoryCitation(
    repoId: string,
    citation: GitHubMemoryCitation
  ): void {
    const parsed = githubMemoryCitationSchema.parse(citation);
    this.db
      .prepare(
        `
        insert into github_memory_citations (
          id, repo_id, conflict_id, source_repo, source_type, number, title, url,
          state, snippet, relevance_reason, confidence, updated_at
        )
        values (
          @id, @repoId, @conflictId, @repo, @type, @number, @title, @url,
          @state, @snippet, @relevanceReason, @confidence, @updatedAt
        )
        on conflict(id) do update set
  ...
  listGitHubMemoryCitations(repoId: string): GitHubMemoryCitation[] {
    const rows = this.db
      .prepare(
        "select * from github_memory_citations where repo_id = ? order by updated_at desc, confidence desc"
      )
      .all(repoId) as GitHubMemoryCitationRow[];
    return rows.map((row) =>
      githubMemoryCitationSchema.parse(githubMemoryCitationFromRow(row))
    );
  }
...
function migrate(db: Database.Database): void {
  db.exec(`
    ...
    create table if not exists github_memory_citations (
      id text primary key,
      repo_id text not null,
      conflict_id text,
...
interface GitHubMemoryCitationRow {
  id: string;
  repo_id: string;
  conflict_id: string | null;
  source_repo: string;
  source_type: GitHubMemoryCitation["type"];
  number: number;
  title: string;
  url: string | null;
  state: string | null;
...
function githubMemoryCitationFromRow(
  row: GitHubMemoryCitationRow
): GitHubMemoryCitation {
  return {
    id: row.id,
