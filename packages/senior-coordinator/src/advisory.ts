import type { Advisory, KiroConflict } from "@kiro/senior-shared";
import { advisorySchema } from "@kiro/senior-shared";
  ...
  const advisory: Advisory = {
    ...
    options: [
      ...
      ...(conflict.classification?.unifiedSpecRecommendation
        ? [
            {
              id: stableId(conflict.id, "unified-spec"),
              title: "Approve unified spec",
              direction: conflict.classification.unifiedSpecRecommendation,
              rationale:
                conflict.classification.blastRadiusSummary ??
                "The model produced a single compatibility spec for both active branches.",
              affectedSurfaces: conflict.affectedSurfaces
            }
          ]
        : []),
