...
import { defaultMonitorBin, findGitRoot } from "@kiro/coordinator";
...
const KIRO_AGENTS_BLOCK = [
  ...
  "- when Kiro returns `directions`, present the role and plan to the user, call `kiro_acknowledge`, then continue from that direction",
...
export async function prepareRuntime(
  ...
  const runtime: KiroRuntime = {
    ...
    monitorBin: existing?.monitorBin ?? defaultMonitorBin()
  };
