...
import { createCoordinatorApp } from "@kiro/coordinator";
...
async function main() {
  ...
  process.env.KIRO_MONITOR_BIN ??= runtime.monitorBin;
  ...
  if (!noDashboard) {
    ...
    if (dashboardProcess) {
      ...
      console.log("Kiro dashboard: run `pnpm --filter @kiro/dashboard dev` from the Kiro repo.");
    }
  }
  ...
  const child = spawn(
    ...
    ],
    {
      env: {
        ...
        KIRO_MONITOR_BIN: process.env.KIRO_MONITOR_BIN ?? runtime.monitorBin
      },
      stdio: "inherit"
    }
  );
