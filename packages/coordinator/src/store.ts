    ...
    this.db
      .prepare(
        ...
        insert into conflicts (
          ...
          affected_files_json, evidence_json, risk_reasons_json, classification_json,
          created_at, updated_at
        )
        values (
          @id, @repoId, @status, @risk, @confidence, @type, @title, @summary,
          @primarySurface, @affectedWorktreeIdsJson, @affectedSurfacesJson,
          @affectedFilesJson, @evidenceJson, @riskReasonsJson, @classificationJson,
          @createdAt, @updatedAt
        )
        on conflict(id) do update set
          status = excluded.status,
      ...
      .run({
        ...
        affectedFilesJson: JSON.stringify(parsed.affectedFiles),
