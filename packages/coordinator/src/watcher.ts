...
import type { TempoConflict, TempoEvent, TempoWorktree } from "@kiro/shared";
...
import { defaultKiroGuardrails } from "./guardrails.js";
...
class ChokidarTempoWatcher implements TempoWatcher {
  ...
  async scanOnce(): Promise<AnalyzeWorktreesResult> {
    ...
    const result = await analyzeWorktreesOnce({
      ...
      rustMonitor: { enabled: true },
      monitorContext: {
        guardrails: defaultKiroGuardrails,
        sessions: this.options.store.listAgentSessions(this.options.repoId).map((session) => ({
          id: session.id,
          worktreeId: session.worktreeId,
          displayName: session.displayName,
          ...(session.currentPlan ? { currentPlan: session.currentPlan } : {})
        }))
      }
    });
    ...
    this.recordEvent("analysis.completed", "Kiro analyzed dirty worktrees", {
      ...
      guardrailViolationCount: result.guardrailViolations.length,
      blastRadiusEventCount: result.blastRadiusEvents.length,
  ...
  private persistAnalysis(result: AnalyzeWorktreesResult): void {
    ...
    for (const worktree of result.worktrees) {
      this.options.store.upsertWorktree({
        ...worktree,
        repoId: this.options.repoId,
        lastObservedAt: this.now()
      });
