...
describe("Kiro MCP tool handlers", () => {
  ...
  it("returns conflict-backed guardrails and blast radius with affected files", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "tempo-mcp-guardrail-"));
    const store = createTempoStore(path.join(dir, "tempo.sqlite"));
    const handlers = createMcpToolHandlers({
      repoId: "repo-1",
      repoRoot: dir,
    ...
    store.upsertConflict({
      ...
      primarySurface: "Forbidden dependency",
      affectedWorktreeIds: [join.worktreeId],
      affectedSurfaces: ["Forbidden dependency"],
    ...
    expect(checkpoint.guardrailViolations).toEqual([
      {
        conflictId: "guardrail-1",
        severity: "medium",
        title: "Forbidden dependency",
        files: ["package.json"],
        evidence: ["Diff matched forbidden pattern `mongodb`."],
    ...
    expect(checkpoint.blastRadiusEvents[0]).toMatchObject({
      conflictId: "guardrail-1",
      files: ["package.json"],
      surfaces: ["Forbidden dependency"]
    });
