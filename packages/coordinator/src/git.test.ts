...
describe("git helpers", () => {
  ...
  it("includes staged-only and untracked files in the worktree diff", async () => {
    const repo = await createRepo();
    await mkdir(path.join(repo, "src", "db"), { recursive: true });
    await writeFile(
      path.join(repo, "src", "db", "schema.ts"),
      "export interface Task { id: string }\n"
    );
    await execa("git", ["add", "src/db/schema.ts"], { cwd: repo });
    await writeFile(
      path.join(repo, "src", "new-contract.ts"),
      "export interface NewContract { id: string }\n"
    );

    const diff = await getWorktreeDiff(repo);

    expect(extractChangedFilesFromDiff(diff)).toEqual([
      "src/db/schema.ts",
      "src/new-contract.ts"
    ]);
    expect(normalizeDiff(diff)).toContain("new file mode");
  });
});
