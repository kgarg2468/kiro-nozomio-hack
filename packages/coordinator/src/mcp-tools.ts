...
export interface CheckpointResult {
  ...
  guardrailViolations: CheckpointGuardrailViolation[];
  blastRadiusEvents: CheckpointBlastRadiusEvent[];
  citations: CheckpointCitation[];
  keepWaiting: boolean;
  pause: boolean;
}

export interface CheckpointGuardrailViolation {
  conflictId: string;
  severity: RiskLevel;
  title: string;
  files: string[];
...
export interface CheckpointBlastRadiusEvent {
  conflictId: string;
  risk: RiskLevel;
  summary: string;
  files: string[];
...
export interface CheckpointCitation {
  label: string;
  sourceType: "guardrail" | "conflict" | "decision";
  snippet: string;
  confidence: "Decided" | "Convention" | "Considered" | "Stale";
}
...
export function createMcpToolHandlers(context: McpToolContext) {
  return {
    ...
    checkpoint(input: CheckpointInput): CheckpointResult {
      ...
      const checkpointConflicts = [...unresolvedConflicts, ...notices];
      ...
      return {
        ...
        guardrailViolations: guardrailViolationsForConflicts(checkpointConflicts),
        blastRadiusEvents: blastRadiusEventsForConflicts(checkpointConflicts),
        citations: citationsForConflicts(checkpointConflicts, activeDecisions),
...
function guardrailViolationsForConflicts(
  conflicts: TempoConflict[]
): CheckpointGuardrailViolation[] {
  return conflicts
    .filter((conflict) => conflict.type === "guardrail" || conflict.type === "stack")
    .map((conflict) => ({
      conflictId: conflict.id,
      severity: conflict.risk,
      title: conflict.title,
      files: conflict.affectedFiles ?? [],
      evidence: conflict.evidence,
      recommendation: conflict.summary
    }));
}

function blastRadiusEventsForConflicts(
  conflicts: TempoConflict[]
): CheckpointBlastRadiusEvent[] {
  return conflicts.map((conflict) => ({
    conflictId: conflict.id,
    risk: conflict.risk,
    summary: conflict.summary,
    files: conflict.affectedFiles ?? [],
    surfaces: conflict.affectedSurfaces,
    recommendation:
      conflict.risk === "high"
        ? "Pause affected agents and agree one owner or compatible contract shape before commit."
        : "Coordinate before commit and keep changes backward-compatible."
  }));
}

function citationsForConflicts(
  conflicts: TempoConflict[],
  decisions: ActiveDecisionBrief[]
): CheckpointCitation[] {
  const conflictCitations = conflicts.flatMap((conflict) =>
    conflict.evidence.slice(0, 3).map((evidence) => ({
      label: conflict.title,
      sourceType:
        conflict.type === "guardrail" || conflict.type === "stack"
          ? ("guardrail" as const)
          : ("conflict" as const),
      snippet: evidence,
      confidence: "Convention" as const
    }))
  );
  const decisionCitations = decisions.map((decision) => ({
    label: decision.selectedOptionTitle,
    sourceType: "decision" as const,
    snippet: decision.selectedOptionDirection,
    confidence: "Decided" as const
  }));
  return [...conflictCitations, ...decisionCitations];
}
